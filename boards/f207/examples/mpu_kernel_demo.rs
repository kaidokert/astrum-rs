//! MPU Kernel Demo — STM32F207ZG (Cortex-M3, no FPU)
//!
//! Two partitions running under MPU enforcement. Proves the kernel's
//! MPU context switch works on Cortex-M3 (no FPU context, no lazy stacking).
//!
//! Build: cd f207 && cargo build --example mpu_kernel_demo \
...
kernel::define_kernel!(MpuConfig, |tick, _k| {
    if tick % 500 == 0 {
        ...
        rprintln!("[{:5}ms] P0={} P1={} MPU_CTRL=0x{:x} CFSR=0x{:x}",
            tick, p0, p1, ctrl, cfsr);
        if p0 > 10 && p1 > 10 && (cfsr & 0xFF) == 0 {
            rprintln!("SUCCESS: MPU enforcement on Cortex-M3! P0={} P1={} CFSR=0", p0, p1);
        }
    }
});
...
#[entry]
fn main() -> ! {
    rprintln!("=== MPU Kernel Demo — STM32F207ZG (Cortex-M3, no FPU) ===");
    ...
    sched.add_system_window(2).expect("SW");
