//! UART DMA Demo — SAME51 Curiosity Nano
//!
//! DMA receive from host via VCOM (SERCOM2, PA13 RX / PA12 TX) into a kernel
//! buffer pool slot, verified by a second partition.
//!
//! Protocol (one cycle):
//!   1. MCU sends "READY\n" via CPU TX
//!   2. Host sends 32-byte pattern (0x00..0x1F)
//!   3. DMAC receives into static buffer (no CPU reads of SERCOM2 DATA)
//!   4. MCU copies DMA result into kernel buffer slot via sys_buf_write
//!   5. Lend to P1; P1 reads and verifies pattern
//!   6. MCU sends "OK\n" or "ERR:...\n" via CPU TX
//!
//! Build: cd same51_curiosity && cargo build --example uart_dma_demo \
...
const SERCOM2_SIZE: u32 = 0x40; // 64 bytes — SERCOM register block is 0x31 bytes
...
kernel::define_kernel!(UartDmaConfig, |tick, _k| {
    if tick % 1000 == 0 {
        ...
        if cycle > 5 && verify_fail == 0 && api_err == 0 {
            rprintln!("SUCCESS: SAME51 UART DMA + buffer pool working! CYCLE={}", cycle);
        }
    }
});
...
// ---------------------------------------------------------------------------
// P0 — UART DMA receiver + buffer pool producer
// ---------------------------------------------------------------------------
extern "C" fn producer_body(_r0: u32) -> ! {
    ...
    loop {
        ...
        // Drain any stale RX bytes before arming DMA.
        // (nEDBG may echo or buffer host data from before READY was sent.)
...
#[entry]
fn main() -> ! {
    rprintln!("=== UART DMA Demo — SAME51 Curiosity Nano ===");
    ...
    rprintln!("[INIT] P0: DMAC+SERCOM2 grants. Pattern: 0x00..0x{:02X}", BUF_LEN as u8 - 1);
