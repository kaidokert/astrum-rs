//! Sampling Port Demo — SAME51 Curiosity Nano
//!
//! Latest-value-wins inter-partition data transfer.
//!
//! - P0 (Producer): writes incrementing counter to sampling port
//! - P1 (Consumer): reads from sampling port (may miss intermediate values)
//!
//! Success: WRITE > 20 && READ > 10.
//!
//! Build: cd same51_curiosity && cargo build --example sampling_demo --features kernel-example

#![no_std]
#![no_main]
#![allow(incomplete_features)]
#![feature(generic_const_exprs)]

use core::sync::atomic::{AtomicU32, Ordering};
use cortex_m_rt::{entry, exception};
use kernel::{
    StackStorage as _,
    partition::{ExternalPartitionMemory, MpuRegion},
    partition_core::AlignedStack2K,
    sampling::PortDirection,
    scheduler::{ScheduleEntry, ScheduleTable},
    svc::Kernel,
    {Partitions2, SyncMinimal, MsgSmall, PortsTiny, DebugEnabled},
};
use rtt_target::rprintln;
use same51_curiosity::{FLASH_BASE, FLASH_SIZE, SRAM_BASE, SRAM_SIZE};

const NUM_PARTITIONS: usize = 2;
const MSG_SIZE: usize = 1;
const DUMMY_IRQ: u8 = 0;
const IRQ_COUNT: usize = 136;

static WRITE_COUNT: AtomicU32 = AtomicU32::new(0);
static READ_COUNT: AtomicU32 = AtomicU32::new(0);

kernel::kernel_config!(SamplingConfig[AlignedStack2K]<Partitions2, SyncMinimal, MsgSmall, PortsTiny, DebugEnabled> {
    core_clock_hz = same51_curiosity::CORE_CLOCK_HZ;
    SW = 2; MS = 2; MW = 2;
    SP = 2; SM = 16; BS = 2; BM = 4; BW = 2;
});

unsafe extern "C" fn dummy_irq() {}
kernel::bind_interrupts!(SamplingConfig, IRQ_COUNT,
    DUMMY_IRQ => (0, 0x0000_0001, handler: dummy_irq),
);

kernel::define_kernel!(SamplingConfig, |tick, k| {
    if tick % 500 == 0 {
        let writes = WRITE_COUNT.load(Ordering::Acquire);
        let reads = READ_COUNT.load(Ordering::Acquire);
        rprintln!("[KERNEL] Tick {}: WRITE={} READ={}", tick, writes, reads);

        if let Some(src) = k.sampling().get(0) {
            rprintln!("  Port 0 (source): size={}, ts={}", src.current_size(), src.timestamp());
        }
        if let Some(dst) = k.sampling().get(1) {
            rprintln!("  Port 1 (dest): size={}, ts={}", dst.current_size(), dst.timestamp());
        }

        if writes > 20 && reads > 10 {
            rprintln!("SUCCESS: Sampling ports working! WRITE={} READ={}", writes, reads);
        }
    }
});

extern "C" fn producer_main_body(r0: u32) -> ! {
    let port_id = r0.into();
    let mut counter: u8 = 0;
    loop {
        let buf = [counter];
        if plib::sys_sampling_write(port_id, &buf).is_ok() {
            WRITE_COUNT.fetch_add(1, Ordering::Release);
        }
        counter = counter.wrapping_add(1);
        for _ in 0..5000 { core::hint::spin_loop(); }
        plib::sys_yield().ok();
    }
}

extern "C" fn consumer_main_body(r0: u32) -> ! {
    let port_id = r0.into();
    let mut read_count: u32 = 0;
    loop {
        let mut buf = [0u8; 1];
        if let Ok(sz) = plib::sys_sampling_read(port_id, &mut buf) {
            if sz > 0 {
                read_count = read_count.wrapping_add(1);
                READ_COUNT.store(read_count, Ordering::Release);
            }
        }
        for _ in 0..3000 { core::hint::spin_loop(); }
        plib::sys_yield().ok();
    }
}

kernel::partition_trampoline!(producer_main => producer_main_body);
kernel::partition_trampoline!(consumer_main => consumer_main_body);

#[entry]
fn main() -> ! {
    rprintln!("\n=== Sampling Port Demo — SAME51 ===");

    let p = cortex_m::Peripherals::take().unwrap();

    let mut sched = ScheduleTable::<{ SamplingConfig::SCHED }>::new();
    sched.add(ScheduleEntry::new(0, 2)).expect("sched P0");
    sched.add(ScheduleEntry::new(1, 2)).expect("sched P1");
    sched.add_system_window(1).expect("sys_window");

    let src_id: u32 = 0;
    let dst_id: u32 = 1;

    static mut STACKS: [AlignedStack2K; NUM_PARTITIONS] = [AlignedStack2K::ZERO; NUM_PARTITIONS];
    let stacks = unsafe { &mut *core::ptr::addr_of_mut!(STACKS) };
    let [ref mut s0, ref mut s1] = *stacks;

    let data_mpu = MpuRegion::new(SRAM_BASE, SRAM_SIZE, 0);
    let code_mpu = MpuRegion::new(FLASH_BASE, FLASH_SIZE, 0);

    let mems = [
        ExternalPartitionMemory::from_aligned_stack(s0, producer_main as kernel::PartitionEntry, data_mpu, kernel::PartitionId::new(src_id as u32))
            .expect("mem0").with_code_mpu_region(code_mpu).expect("code0"),
        ExternalPartitionMemory::from_aligned_stack(s1, consumer_main as kernel::PartitionEntry, data_mpu, kernel::PartitionId::new(dst_id as u32))
            .expect("mem1").with_code_mpu_region(code_mpu).expect("code1"),
    ];

    let mut k = Kernel::<SamplingConfig>::new(sched, &mems).expect("kernel");
    store_kernel(&mut k);

    kernel::state::with_kernel_mut::<SamplingConfig, _, _>(|k| {
        let s = k.sampling_mut().create_port(PortDirection::Source, 100).expect("source port");
        let d = k.sampling_mut().create_port(PortDirection::Destination, 100).expect("dest port");
        k.sampling_mut().connect_ports(s, d).expect("connect ports");
        Ok::<(), ()>(())
    }).expect("ipc setup");

    rprintln!("[INIT] Booting...\n");
    match boot(p).expect("boot") {}
}
