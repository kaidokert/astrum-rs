//! MPU Kernel Demo — SAME51 Curiosity Nano
//!
//! Two partitions running under MPU enforcement. Validates that the kernel's
//! dynamic MPU region programming works on the ATSAME51J20A (Cortex-M4F,
//! PMSAv7 — same MPU architecture as STM32F4).
//!
//! Build: cd same51_curiosity && cargo build --example mpu_kernel_demo --features kernel-mpu
...
kernel::bind_interrupts!(MpuConfig, IRQ_COUNT,
...
kernel::define_kernel!(MpuConfig, |tick, _k| {
    if tick % 500 == 0 {
        ...
        if p0 > 10 && p1 > 10 && (cfsr & 0xFF) == 0 {
            rprintln!("SUCCESS: SAME51 MPU enforcement working! P0={} P1={} MemManage=0", p0, p1);
        }
    }
});
...
#[entry]
fn main() -> ! {
    rprintln!("=== MPU Kernel Demo — SAME51 Curiosity Nano ===");
