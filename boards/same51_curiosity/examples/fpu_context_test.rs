...
//!
//! Build: cd same51_curiosity && cargo build --example fpu_context_test --features kernel-fpu
...
kernel::kernel_config!(FpuTestCfg[AlignedStack2K]<Partitions2, SyncMinimal, MsgMinimal, PortsTiny, DebugEnabled> {
    core_clock_hz = same51_curiosity::CORE_CLOCK_HZ;
});

unsafe extern "C" fn dummy_irq() {}

kernel::bind_interrupts!(FpuTestCfg, IRQ_COUNT,
...
fn main() -> ! {
    ...
    let mem1 = ExternalPartitionMemory::from_aligned_stack(
            s1, p1_main as kernel::PartitionEntry, data_mpu, kernel::PartitionId::new(0)
        )
        .expect("mem1")
        .with_code_mpu_region(code_mpu)
        .expect("code1");
    ...
    let mems = [mem0, mem1];
    let mut k = Kernel::<FpuTestCfg>::new(sched, &mems).expect("kernel");
    store_kernel(&mut k);

    rprintln!("\n=== FPU Context Switch Test — SAME51 (Cortex-M4F) ===");
