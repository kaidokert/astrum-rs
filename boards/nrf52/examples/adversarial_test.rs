//! Adversarial Multi-Attack Test — nRF52833
//!
...
//!
//! Static MPU layout:
//!   R0: Flash 512 KB (0x0000_0000) — RO+X unprivileged
//!   R1: RAM 128 KB (0x2000_0000) — RW+XN unprivileged
//!   R2: Guard 32 KB (0x2001_8000) — priv-only (top of SRAM, covers no partition data)
//!
//! Success: P0 count > 200 while P1, P2, P3 are all Faulted.
//!
//! Build: cd nrf52 && cargo build --example adversarial_test --features kernel-example
...
/// Guard region at top of 128 KB SRAM — no partition data here.
const GUARD_BASE: u32 = 0x2001_8000;
const GUARD_SIZE: u32 = 32 * 1024;
const P1_TARGET: u32 = 0x2001_8800; // write to guard
const P2_TARGET: u32 = 0x2001_8400; // read from guard
const P3_TARGET: u32 = 0x2001_8C00; // read from guard (different offset)
...
fn configure_static_mpu(mpu_periph: &cortex_m::peripheral::MPU) {
    ...
    // R0: Flash 512 KB (0x0000_0000) — RO+X unprivileged
    let sf = mpu::encode_size(512 * 1024).expect("flash");
    ...
    // R1: RAM 128 KB (0x2000_0000) — RW+XN unprivileged
    let sf = mpu::encode_size(128 * 1024).expect("ram");
