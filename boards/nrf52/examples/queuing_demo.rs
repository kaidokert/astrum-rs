//! Queuing Port Demo for nRF52833 (PCA10100)
//!
...
extern "C" fn commander_main_body(r0: u32) -> ! {
    let ports = r0;
    let cmd_port = (ports >> 16).into();  // Send commands
    let rsp_port = (ports & 0xFFFF).into(); // Receive responses
...
extern "C" fn worker_main_body(r0: u32) -> ! {
    let ports = r0;
    let cmd_port = (ports >> 16).into();  // Receive commands
    let rsp_port = (ports & 0xFFFF).into(); // Send responses
...
fn main() -> ! {
    ...
    rprintln!("nRF52833 - Command/Response Pattern");
