...
fn main() -> ! {
    rprintln!("=== Mutex Demo ===");
    rprintln!("STM32F429ZI - Mutual Exclusion and Critical Sections");
    ...
    // Mutexes are pre-created in MutexPool::new(MS)
    // We have MS=2 mutexes: ID 0 and ID 1
    // Use mutex 0 for shared counter protection
    let mtx = MutexId::new(0);

    // Both partitions get the same mutex ID
    let h: [u32; NUM_PARTITIONS] = [mtx.as_raw(), mtx.as_raw()];
    ...
    rprintln!("Using mutex: {} (from {} available)", mtx.as_raw(), MutexConfig::MS);
