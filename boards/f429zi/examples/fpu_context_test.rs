...
//!
//! Build: cd f429zi && cargo build --example fpu_context_test --features kernel-fpu
//!
//! This test exercises:
//! - Hardware FPU enable (CPACR CP10/CP11)
//! - FPCCR lazy stacking (LSPEN + ASPEN)
//! - s0-s15 hardware-saved registers (exception frame)
//! - s16-s31 software-saved registers (PendSV must save/restore these)
...
fn fpu_load_and_verify(base: u32, pass_counter: &AtomicU32, fail_counter: &AtomicU32) {
    unsafe {
        // Load s0-s15 (hardware-saved on exception) and s16-s31 (software-saved)
        core::arch::asm!(
            "vmov s0, {base}",
            // s0 = base | 0
            "orr {tmp}, {base}, #1",
...
fn main() -> ! {
    ...
    rprintln!("\n=== FPU Context Switch Test — STM32F429ZI (Cortex-M4F) ===");
