//! STM32F429ZI: Context switching with dynamic MPU (Step 1b)
//!
//! Builds on Step 1a by moving MPU reconfiguration into PendSV.
//! True isolation: Partition 0 can ONLY access its own stack, not partition 1's.
//!
//! Step 1b: MPU reconfiguration in PendSV during context switch
//! - Region 2 is reprogrammed atomically during switch
//! - Partition can only access its own stack, not the other partition's
//! - Closer to kernel's approach
        ...
        if current != u32::MAX {
            *SWITCH_COUNT += 1;
            ...
            if count >= TARGET_SWITCHES {
                rprintln!("\ncontext_switch_mpu_dynamic: {} switches observed - PASS", TARGET_SWITCHES);
                rprintln!("MPU region 2 reconfigured {} times in PendSV", count);
                rprintln!("True partition isolation achieved");
                *DONE = true;
                return;
            }
        }
...
fn main() -> ! {
    ...
    rprintln!("STM32F429ZI Context Switch + Dynamic MPU (Step 1b)");
    ...
    // Configure initial MPU (Step 1b: Region 2 will be reconfigured in PendSV)
    rprintln!("Configuring MPU (Step 1b - dynamic region 2):");
    ...
    rprintln!("Region 2 will be reprogrammed on each switch");
    rprintln!("TRUE ISOLATION: Partition can only access its own stack");
