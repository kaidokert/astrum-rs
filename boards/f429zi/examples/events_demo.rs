...
//!
//! Demonstrates inter-partition event signaling on STM32F429ZI.
//!
//! Pattern:
...
//!
//! Build:  cd f429zi && cargo build --example events_demo --features kernel-example
...
use plib::{EventMask, PartitionId};
...
///
/// hint = P1_PID (the partition to signal)
extern "C" fn setter_main_body(r0: u32) -> ! {
    let target_pid = r0.into(); // = P1_PID = 1
...
extern "C" fn waiter_main_body(_r0: u32) -> ! {
    ...
    loop {
        ...
        //   - Ok(0): was put to Waiting; just woke up. Call again to consume.
        //   - Ok(bits): matched bits returned, event consumed.
        //   - Err(_): invalid call — should never happen.
