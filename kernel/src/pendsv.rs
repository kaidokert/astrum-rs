...
mod tests {
    ...
    /// Verify PendSV assembly offset invariants at test time for a concrete config.
    ///
    // TODO: This test re-calculates offsets with offset_of! rather than verifying
    // the actual KERNEL_*_OFFSET / CORE_*_OFFSET statics generated by
    // define_unified_kernel!, because those statics are only emitted inside
    // the macro expansion scope (which requires ARM-only cortex-m intrinsics).
    // The authoritative compile-time assertions live in the macro's const block.
    #[test]
    fn pendsv_offset_invariants() {
        ...
        compose_kernel_config!(TestPendsvCfg<Partitions2, SyncMinimal, MsgMinimal, PortsTiny, DebugEnabled>);

        type K = Kernel<TestPendsvCfg>;
        ...
        assert!(
            kco < 4095,
            "KERNEL_CORE_OFFSET={kco} exceeds Thumb2 ldr range"
        );
        assert!(
            cnp < 4095,
            "CORE_NEXT_PARTITION_OFFSET={cnp} exceeds Thumb2 ldr range"
        );
        assert!(
            csp < 4095,
            "CORE_PARTITION_SP_OFFSET={csp} exceeds Thumb2 ldr range"
        );

        // Field ordering invariants.
        assert!(
            kcp < kco,
        ...
        // partition_sp element stride is exactly 4 bytes.
        // Verify through the actual Kernel struct field, not a hardcoded u32.
        let kernel = K::default();
        assert_eq!(
            core::mem::size_of_val(&kernel.core.partition_sp[0]),
            4,
