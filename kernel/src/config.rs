...
/// model floor.  Panics if `priority` is numerically below (higher urgency
/// than) `min_app_irq_priority`.
///
...
#[inline]
pub fn validate_irq_priority(priority: u8, min_app_irq_priority: u8) {
    assert!(
        priority >= min_app_irq_priority,
        "IRQ priority violates MIN_APP_IRQ_PRIORITY floor"
    );
}
...
mod tests {
    ...
    fn validate_irq_priority_accepts_valid() {
        ...
        validate_irq_priority(0xC0, 0x20);
    }

    #[test]
    #[should_panic(expected = "MIN_APP_IRQ_PRIORITY")]
    fn validate_irq_priority_rejects_below_floor() {
        // 0x10 < 0x20 — violates the floor.
        validate_irq_priority(0x10, 0x20);
    }
}
