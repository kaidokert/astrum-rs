    ...
    >,
{
    ...
    crate::state::with_kernel_mut::<C, _, _>(|k| {
        ...
        #[allow(clippy::manual_inspect)]
        validate_preconfigured_pcbs(k.partitions().as_slice(), C::MPU_ENFORCE).map_err(|e| {
            crate::klog!("boot: {}", e);
            e
        })?;
        ...
        for i in 0..n {
            let pcb = k.partitions().get(i).ok_or_else(|| {
                let e = BootError::StackInitFailed { partition_index: i };
                crate::klog!("boot: {}", e);
                e
            })?;
            ...
            let sp = u32::try_from(ix)
                ...
                .ok_or_else(|| {
                    let e = BootError::StackSizeOverflow { partition_index: i };
                    crate::klog!("boot: {}", e);
                    e
                })?;
            k.set_sp(i, sp);
        ...
        for i in 0..n {
            if let Some(pcb) = k.partitions_mut().get_mut(i) {
                crate::mpu::precompute_mpu_cache(pcb).map_err(|e| {
                    let be = BootError::MpuCachePopulationFailed {
                        partition_index: i,
                        source: e,
                    };
                    crate::klog!("boot: {}", be);
    ...
    // Step 6: Initialize MPU hardware.
    {
        ...
        // SAFETY: __boot_mpu_init is generated by define_harness!
        // and called exactly once before the first context switch.
        unsafe { __boot_mpu_init(&peripherals.MPU) }.map_err(|reason| {
            let e = BootError::BootMpuInitFailed { reason };
            crate::klog!("boot: {}", e);
            e
        })?;
    }
...
mod tests {
    ...
    fn store_kernel_ptr_sets_valid_pointer_for_isr_access() {
        use crate::{
            kernel_config, kernel_ptr, DebugEnabled, MsgMinimal, Partitions2, PortsTiny,
            SyncMinimal,
        };
        kernel_config!(BootTestConfig<Partitions2, SyncMinimal, MsgMinimal, PortsTiny, DebugEnabled>);
    ...
    fn r0_hint_propagates_through_with_config() {
        ...
        kernel_config!(R0Config<Partitions2, SyncMinimal, MsgMinimal, PortsTiny, DebugEnabled>);
