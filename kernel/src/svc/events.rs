...
mod tests {
    ...
    fn wait_acquired_and_blocked_paths() {
        let mut t = pt();
        handle_event_set(&mut t, 0, 0b1010);
        let (val, block) = handle_event_wait(&mut t, 0, 0b1110);
        ...
        let (val, block) = handle_event_wait(&mut t, 0, 0b0011);
    ...
    fn set_wakes_and_clear_returns_previous() {
        let mut t = pt();
        let (_, block) = handle_event_wait(&mut t, 0, 0b0001);
        assert!(block);
        assert_eq!(handle_event_set(&mut t, 0, 0b0001), 0);
        assert_eq!(t.get(0).unwrap().state(), PartitionState::Ready);
        assert_eq!(t.get(0).unwrap().event_flags(), 0b0001);
        handle_event_set(&mut t, 1, 0b1111);
        assert_eq!(handle_event_clear(&mut t, 1, 0b0101), 0b1111);
    ...
    fn invalid_partition_targets() {
        ...
        let (val, block) = handle_event_wait(&mut t, 99, 0b1);
        assert_eq!(val, inv);
        assert!(!block, "must not block on invalid partition");
        assert_eq!(handle_event_set(&mut t, 99, 0b1), inv);
        assert_eq!(handle_event_clear(&mut t, 99, 0b1), inv);
    }
}
