...
>(
    ...
    match pool.receive_queuing_message(port_id.as_raw() as usize, current_partition, buf, 0, tick) {
...
>(
    ...
    match pool.get_queuing_port_status(port_id.as_raw() as usize) {
        Ok(status) => {
...
mod tests {
    ...
    fn recv_and_status_outcomes() {
        ...
        assert_eq!(unsafe { handle_queuing_status(&pool, qid(99), s2.as_mut_ptr()) }, SvcError::InvalidResource.to_u32());
    }
    ...
    fn qid(raw: usize) -> QueuingPortId {
        QueuingPortId::new(raw as u32)
    }
    ...
    fn newtype_id_roundtrip() {
        ...
        let s = unsafe { s.assume_init() };
        assert_eq!((s.max_nb_messages, s.max_message_size), (2, 64));
    }
}
