...
mod tests {
    ...
    fn write_read_roundtrip() {
        ...
        unsafe {
            assert_eq!(
                handle_buf_write(&mut p, BufferSlotId::new(0), 0, data.as_ptr(), 4),
                (4, None)
            );
            let mut out = [0u8; 4];
            assert_eq!(
                handle_buf_read(&mut p, BufferSlotId::new(0), 0, out.as_mut_ptr(), 4),
                (4, None)
            );
            assert_eq!(out, data);
        }
    ...
    fn slot_id_roundtrip() {
        ...
        let id = BufferSlotId::new(handle_buf_alloc(&mut p, 0, 1, 0, 0).0 as u8);
        // SAFETY: stack-local slices valid for the call duration.
        unsafe { assert_eq!(handle_buf_write(&mut p, id, 0, data.as_ptr(), 3).0, 3) };
        let mut out = [0u8; 3];
        // SAFETY: stack-local slices valid for the call duration.
        unsafe { assert_eq!(handle_buf_read(&mut p, id, 0, out.as_mut_ptr(), 3).0, 3) };
        assert_eq!(out, data);
        assert_eq!(handle_buf_release(&mut p, id, 0), (0, None));
    }
    ...
    fn write_read_errors() {
        ...
        unsafe {
            assert_eq!(
                handle_buf_write(&mut p, BufferSlotId::new(0), 1, [1u8].as_ptr(), 1).0,
                perm
            );
            assert_eq!(
                handle_buf_write(&mut p, BufferSlotId::new(99), 0, [1u8].as_ptr(), 1).0,
                inv
            );
            assert_eq!(
                handle_buf_write(&mut p, BufferSlotId::new(1), 1, [1u8].as_ptr(), 1).0,
                perm
            );
            // read from invalid slot → error with discriminant
            let (r0, r1) =
                handle_buf_read(&mut p, BufferSlotId::new(99), 0, [0u8; 4].as_mut_ptr(), 4);
