...
kernel::define_unified_harness!(TestConfig, |tick, _k| {
    ...
    if tick == 2 || tick == 6 || tick == 10 {
        ...
        // (not an instance method); no `unsafe` block required. Same pattern
        // used in irq_ack_test.rs, irq_dispatch_test.rs, irq_multi_test.rs.
        #[cfg(target_arch = "arm")]
...
fn main() -> ! {
    ...
    // TODO: reviewer false positive — store_kernel/boot are generated by
    // define_unified_harness!, enable_bound_irqs by bind_interrupts!; they
    // are not crate imports (same pattern as irq_ack_test.rs).
    store_kernel(k);
