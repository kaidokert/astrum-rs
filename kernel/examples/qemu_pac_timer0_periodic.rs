//! QEMU proof-of-concept: partition exercises TIMER0 in 32-bit periodic mode via PAC typed API.
//!
//! Privileged main() enables TIMER0 clock via SYSCTL RCGC1 bit 16, static MPU grants P0
//! access to TIMER0 (0x4003_0000, 4 KiB). P0 configures TIMER0 in 32-bit periodic mode
//! (CFG=0, TAMR=periodic), loads TAILR with a reload value, enables via CTL.
//! Verifies config registers read back correctly, TAR is readable, CTL shows enabled.
//!
//! Run: cargo run --example qemu_pac_timer0_periodic --features qemu,log-semihosting,qemu-peripherals \
...
static TIMER_TAR: AtomicU32 = AtomicU32::new(0);
...
/// P0: configure TIMER0 in 32-bit periodic mode, read registers, then set state flag.
...
extern "C" fn p0_entry() -> ! {
    ...
    t.cfg.write(|w| w.timer_cfg().timer_cfg_32_bit_timer());
    t.tamr
        .write(|w| w.timer_tamr_tamr().timer_tamr_tamr_period());
    // SAFETY: Valid 16-bit values for TAILR low/high halves.
    t.tailr.write(|w| unsafe {
        w.timer_tailr_tailrl()
            .bits(RELOAD_VALUE as u16)
            .timer_tailr_tailrh()
            .bits((RELOAD_VALUE >> 16) as u16)
    });
    t.ctl.write(|w| w.timer_ctl_taen().set_bit());

    for _ in 0..100u32 {
        cortex_m::asm::nop();
    }

    TIMER_TAR.store(t.tar.read().bits(), Ordering::Release);
...
kernel::define_kernel!(no_boot, TestConfig, |tick, _k| {
    if TIMER_STATE.load(Ordering::Acquire) == 1 {
        ...
        hprintln!(
            "qemu_pac_timer0_periodic: TAR={:#010x} CTL={:#06x} TAILR={:#010x} CFG={} TAMR={}",
            tar,
            ctl,
            tailr,
            cfg,
        ...
        if ctl & 1 == 0 {
            hprintln!("qemu_pac_timer0_periodic: FAIL CTL.TAEN not set");
            kernel::kexit!(failure);
        }
        if cfg != 0 {
            hprintln!("qemu_pac_timer0_periodic: FAIL CFG={} expected 0", cfg);
            kernel::kexit!(failure);
        }
        if tamr & 0x3 != 2 {
            hprintln!("qemu_pac_timer0_periodic: FAIL TAMR={} expected 2", tamr);
            kernel::kexit!(failure);
        }
        if tailr != RELOAD_VALUE {
            hprintln!("qemu_pac_timer0_periodic: FAIL TAILR mismatch");
            kernel::kexit!(failure);
        }
        // TAR read succeeded without MemManage fault (proves MPU grant works).
        // TODO(qemu): QEMU's Stellaris GPTM does not model TAR countdown, so TAR
        // reads back as 0. On real hardware TAR should be non-zero after enabling
        // a periodic timer with a non-zero reload value. We check anyway so this
        // test will validate the countdown on a faithful emulator or real silicon.
        if tar == 0 {
            hprintln!("qemu_pac_timer0_periodic: WARN TAR=0 (QEMU does not model countdown)");
        } else if tar > RELOAD_VALUE {
            hprintln!(
                "qemu_pac_timer0_periodic: FAIL TAR={:#010x} exceeds RELOAD",
                tar
            );
            kernel::kexit!(failure);
        }
        hprintln!("qemu_pac_timer0_periodic: PASS TIMER0 32-bit periodic mode OK");
        kernel::kexit!(success);
    }
    if tick >= 200 {
        hprintln!("qemu_pac_timer0_periodic: FAIL timeout");
        kernel::kexit!(failure);
    }
});
...
fn main() -> ! {
    ...
    if configure_static_mpu(&p.MPU).is_none() {
        hprintln!("qemu_pac_timer0_periodic: FATAL MPU config failed");
        fatal_halt();
    }
    ...
    static TIMER0_REGION: [MpuRegion; 1] = [MpuRegion::new(TIMER0_BASE, TIMER0_SIZE, 0)];
    let mut k = {
