...
fn main() -> ! {
    ...
    let mut sched = ScheduleTable::<{ ClearBitConfig::SCHED }>::round_robin(1, 3)
    ...
    let mut k = init_kernel(sched, &parts).expect("irq_clearbit_test: init_kernel");
    store_kernel(&mut k);
    ...
    match boot(p).expect("irq_clearbit_test: boot") {}
}
