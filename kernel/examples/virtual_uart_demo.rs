...
fn main() -> ! {
    ...
    let mems: [ExternalPartitionMemory; NUM_PARTITIONS] = {
        ...
        [
            ExternalPartitionMemory::new(&mut s0.0, entry_point_addr(p1_main), sentinel_mpu, 0)
                .expect("ext mem"),
            ExternalPartitionMemory::new(&mut s1.0, entry_point_addr(p2_main), sentinel_mpu, 1)
                .expect("ext mem"),
        ]
    };
