...
fn main() -> ! {
    ...
    static SSI0_REGION: [MpuRegion; 1] = [MpuRegion::new(SSI0_BASE, SSI0_SIZE, 0)];
    ...
    let mut k = {
        ...
        let memories: [_; NP] = core::array::from_fn(|i| {
            ...
            let spec = PartitionSpec::entry(entry_fns[i])
                ...
                .with_peripherals(if i == 0 { &SSI0_REGION } else { &[] });
    ...
    // store_kernel is generated by define_kernel! (via define_unified_kernel!).
    store_kernel(&mut k);
