...
fn main() -> ! {
    ...
    let mem0 =
        ExternalPartitionMemory::new(&mut stack0.0, entry_point_addr(p0_main), sentinel_mpu, 0)
            .expect("ext mem 0");
    let mem1 =
        ExternalPartitionMemory::new(&mut stack1.0, entry_point_addr(p1_main), sentinel_mpu, 1)
            .expect("ext mem 1");
