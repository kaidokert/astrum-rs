...
`define_kernel!` macro for boot sequencing. The typical setup:
...
fn main() -> ! {
    ...
    // SAFETY: STACKS is a static array generated by define_kernel!
...
SVC dispatch via `define_kernel!`, PendSV context switching,
...
`define_kernel!`, PendSV context switching, SysTick scheduling,
...
(which use `define_kernel!`), this test uses a custom SysTick
...
| `svc_yield` | SVC handler integration -- links the kernel SVC trampoline via `define_kernel!` and dispatches a yield. |
...
- `SYS_DEV_OPEN` / `SYS_DEV_READ_TIMED` / `SYS_DEV_WRITE` /
  `SYS_YIELD` syscalls via the `define_kernel!` macro
