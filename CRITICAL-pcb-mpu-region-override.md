# CRITICAL: PCB MPU Region Override — Stale Pre-Move Stack Address
...
`Kernel::new()` (`svc.rs:1043-1057`) delegates to `with_config()`, which
copies each `PartitionConfig.mpu_region` directly into the PCB
...
The boot sequence applies a post-move fixup that calls
`fix_mpu_data_region()` **only** for sentinel (size==0) partitions.
User-configured partitions (size > 0) keep their original base untouched.
...
3. **Boot fixup loop** (`boot.rs:310-321`) — After stack relocation, calls
   `fix_mpu_data_region_if_sentinel()` with the real `core_stack` base.

4. **`Kernel::new()`** (`svc.rs:1043-1057`) — Simplified passthrough to
   `with_config()`; fixup deferred to boot time.

5. **Invariant check** (`boot.rs:327-336`) — Debug-only
   `assert_pcb_addresses_in_storage()` validates all PCB addresses fall
   within live storage after the fixup loop completes.
...
| Test | Line | Purpose |
|------|------|---------|
| `fix_mpu_data_region_preserves_peripheral_regions` | 1000 | Peripheral regions untouched by fixup |
| `fix_mpu_data_region_user_configured_preserves_all_fields` | 1036 | Non-sentinel fields fully preserved |
