//! Debug logging types and constants shared between kernel and partition code.
//!
//! This module contains the `DebugRingBuffer` and related types used for
//! partition-to-kernel debug communication. These types reside in the shared
//! traits crate to avoid architectural layering violations (plib should not
...
/// where the partition is the producer (calling `write`/`write_record`) and
/// the kernel is the consumer (calling `drain`).
///
/// # Size Requirement
///
/// `N` must be a power of 2. This is enforced at compile time.
